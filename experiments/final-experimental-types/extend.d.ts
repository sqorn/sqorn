...
import { States, Keys, Query, RequiredKeys, UnionToIntersection } from './queries'

type Combine<A, B> = (A & B & RequiredKeys extends never ? never : (A & B))
// type ValidBuilder<T> = (States<T> & RequiredKeys) extends never ? never : T;

interface meow<T> { n(): T }

type x = meow<1> extends meow<1 | (2 & 3)> ? true : false

// declare let x: never, y: never, z: number;
// x = z;
export interface Extend<T0 extends Keys> {
  extend<
    Q1 extends (UnionToIntersection<RequiredKeys> extends UnionToIntersection<States<Q1> & T0 & RequiredKeys> ? unknown : never),
    >(
      q1: Q1
    ): Next<T0, States<Q1>, never>
