...
describe('set', () => {
  describe('template string', () => {
    query({
      name: '1 column',
      qry: sq.set`age = age + 1`,
      txt: 'set age = age + 1'
    })
    query({
      name: '2 columns',
      qry: sq.set`age = age + 1, updated = now()`,
  ...
  describe('object', () => {
    query({
      name: '1 column',
      qry: sq.set({ age: sq.l`age + 1` }),
      txt: 'set age = age + 1'
    })
    query({
      name: '2 columns',
      qry: sq.set({ age: sq.l`age + 1`, updated: sq.l`now()` }),
      txt: 'set age = age + 1, updated = now()'
    })
    query({
      name: '1 parameterized arg',
