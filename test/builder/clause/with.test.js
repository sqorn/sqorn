...
describe('with', () => {
  ...
  describe('object - subquery arg', () => {
    ...
    query({
      ...
      text: 'with a as (update b set x = $1) select * from a',
      args: [1]
    })
    query({
    ...
    query({
      ...
      text: 'with a as (values ($1, $2), ($3, $4)) select * from a',
      args: [1, 2, 3, 4]
    })
    query({
      ...
      text:
        'with a as (select $1, $2), b as (select $3, $4) select * from a union all b',
      args: [1, 2, 3, 4]
    })
    query({
      name: 'two objects',
  ...
  describe('recursive', () => {
    query({
      ...
      query: sq.recursive.with({
        't(n)': sq.values`(1)`.union.all(
          sq.l`select n + 1 from t where n < 100`
        )
      }).return`sum(n)`,
      text:
        'with recursive t(n) as (values (1) union all (select n + 1 from t where n < 100)) select sum(n)'
    })
  })
})
