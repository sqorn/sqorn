...
describe('pg', async () => {
  beforeAll(async () => {
    ...
    await sq.sql`drop database if exists ${sq.raw(db_name)}`.run()
    await sq.sql`create database ${sq.raw(db_name)}`.run()
    await sq.end()
  })
  ...
  test('db empty', async () => {
    expect(
      await sq.from`pg_catalog.pg_tables`.where`schemaname = 'public'`.all()
    ).toEqual([])
  })
  ...
  test('insert authors', async () => {
    expect(
      await sq.from`author`.return`first_name, last_name`
        .insert(
          {
            firstName: 'Brandon',
            lastName: 'Sanderson',
  ...
  test('insert books', async () => {
    expect(
      await sq.from`book`.return`title, publish_year`
        .insert(
          {
  ...
  test('select author', async () => {
    expect(
      await sq.from`author`.where({ firstName: 'Brandon' })
        .return`last_name`.one()
  ...
  test('select book', async () => {
    expect(
      await sq.from`book`.where({ genre: 'Adventure' }).return`id`.first()
    ).toEqual({ id: 1 })
  })
  test('delete book', async () => {
    expect(
      await sq.from`book`.delete.return`id`
        .where({ title: 'The Way of Kings' })
        .first()
    ).toEqual({ id: 1 })
  })
  test('select book', async () => {
    expect(await sq.from`book`.return`id`.all()).toEqual([{ id: 2 }, { id: 3 }])
  })
  test('transaction callback success', async () => {
    const { jo, mo } = await sq.transaction(async trx => {
      const jo = await sq.from`author`.insert({ firstName: 'Jo' })
        .return`*`.one(trx)
      const [mo] = await sq.from`author`.insert({ firstName: 'Mo' })
        .return`*`.all(trx)
      return { jo, mo }
    })
    expect(jo.firstName).toBe('Jo')
    expect(mo.firstName).toBe('Mo')
    const authors = await sq.from`author`.all()
    ...
    expect(
      (async () => {
        await sq.transaction(async trx => {
          await sq.from`author`.insert({ firstName: 'Bo' }).return`*`.one(trx)
          throw Error('oops')
        })
      })()
    ).rejects.toThrow('oops')
    const bo = await sq.from`author`.where({ firstName: 'Bo' }).first()
    expect(bo).toBe(undefined)
  })
  test('transaction value success', async () => {
    const trx = await sq.transaction()
    const ma = await sq.from`author`.insert({ firstName: 'Ma' }).return`*`.one(
      trx
    )
    const pa = await sq.from`author`.insert({ firstName: 'Pa' }).return`*`.one(
      trx
    )
    await trx.commit()
  ...
  test('transaction value rollback', async () => {
    const trx = await sq.transaction()
    let da = await sq.from`author`.insert({ firstName: 'Da' }).return`*`.one(
      trx
    )
    expect(da.firstName).toBe('Da')
    await trx.rollback()
    da = await sq.from`author`.where({ firstName: 'Da' }).first()
